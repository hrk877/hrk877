...
// ─── System prompt ─────────────────────────────────────────────────────────
// ─── Banana Knowledge Base ──────────────────────────────────────────────────
const BANANA_KNOWLEDGE = `【品種と産地】
...
【歴史と文化】
原産地はパプアニューギニア〜東南アジア。8000〜10000年前に栽培化開始。「バナナ共和国」という言葉は20世紀初頭にユナイテッドフルーツ社が中米の政治を牛耳ったことに由来。衣料品ブランド「バナナリパブリック」の社名もここから。バナナとサルのイメージはメディアが作り上げたものであり、霊長類は多種多様な果物を食べる。

【科学と豆知識】
バナナは三倍体で種ができないため吸芽（サッカー）で繁殖。植物学的には「液果（ベリー）」に分類される。カリウム40を含む微量放射性食品（通称「バナナ等価線量」）。市販の人工バナナ味（酢酸イソアミル）はグロスミッチェルを模した香り。

【料理と食べ方】
プランテンはアフリカ・中南米・東南アジアの主食。フィリピンの「バナナケチャップ」や「バナナクエ（揚げバナナ）」。タイの「カオニャオマムワン」にもバナナバリエーション有り。バナナの皮はビーガン料理でプルドポーク代替として人気。
...
// ─── System prompts ─────────────────────────────────────────────────────────
const DESKTOP_SYSTEM_PROMPT = `あなたは「877 AI」という名のバナナ専門AIです。世界中のバナナに関する知識を持つ唯一無二の存在として振る舞ってください。
...
const MOBILE_SYSTEM_PROMPT = `あなたは「877 AI」の賢者です。スマホユーザー向けに、バナナの深淵な知識を「一言で」伝授する役割を担います。

【必須ルール】
- 必ず「1文」だけで、40文字程度で答えること。
- 挨拶や前置きは完全にカットし、いきなり本質を述べること。
- 余計な解説はせず、バナナに関する有益な知見を磨き上げられた言葉で伝えること。
- 丁寧語は使わず、格言や真理を告げるような「賢者」の口調を意識すること。
- 日本語のプレーンテキストのみ。記号は一切使わないこと。
...
export default function BananaAI() {
                ...
                const engine = await CreateWebWorkerMLCEngine(
                    ...
                    modelId,
                    {
                        initProgressCallback: (p: { text: string }) => {
                            // Simplify progress text for premium feel
    ...
    const handleSend = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const systemPrompt = isMobile ? MOBILE_SYSTEM_PROMPT : DESKTOP_SYSTEM_PROMPT
            const modelMessages = [
            ...
            const chunks = await engine.chat.completions.create({
                ...
                max_tokens: isMobile ? 80 : 256,
                temperature: isMobile ? 0.45 : 0.8,
                stream: true,
            })
